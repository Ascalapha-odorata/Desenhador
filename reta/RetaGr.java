...
public class RetaGr extends Reta{
    ...
    public RetaGr(PontoGr p1, PontoGr p2, Color cor, String str, int esp){
        super(p1, p2);
        setCorReta(cor);
        setNomeReta(str);
        setEspReta(esp);
    ...
    public void desenharReta(Graphics g){
        ...
        if(dX>dY){//se delta X for maior, calcula variando o x
            // percorre de x1 ate x2. 
            // y e´ calculado pela equacao: y = mx + b
            pIni = p1.getX();
            pFim = p2.getX();
            if(pIni > pFim){//se a coordenada inicial for maior, troca a ordem
                aux = pIni;
                pIni = pFim;
                pFim = aux;
            }
        ...
        } else{ //se delta Y for maior, calcula variando o y
            //percorre y1 ate y2
            //x e' calculado pela equacao: x = (y - b)/m
            pIni = p1.getY();
            pFim = p2.getY();
            if(p1.getX() == p2.getX()){//se a reta for vertical (x = constante)
                //os pontos da reta não variam o valor de x
                x = p1.getX();
                for(y = pIni; y <= pFim; y++){ 
                    // Define ponto grafico
                    ponto = new PontoGr((int)x, (int)y, getCorReta(), getEspReta());
